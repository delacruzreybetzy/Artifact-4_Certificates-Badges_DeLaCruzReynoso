# Artifact 4: Certificate & Badge Documentation

## Student Information
- **Name:** Betzy De La Cruz Reynoso
- **Course:** Study Skills (HUB)
- **Artifact:** 4 — Certificates & Badges

## Certificate Details
- **Certificate Title:** Network Addressing and Basic Troubleshooting
- **Issuing Organization:** Cisco Networking Academy
- **Total Hours Completed:** 14 hours
- **Completion Date:** 03/17/2026
- **Badge Platform:** Cisco Badge / Credly
- **Badge Link:** https://www.credly.com/badges/62375170-1b93-48b1-a505-8bcbde5a8393/public_url


## Files Included in This Repository
- Certificate PDF 
- Badge Image/Screenshot
- This README.md file documenting certificate & skills

## Skills Learned
- IPv4 and IPv6 addressing and subnetting
- Network troubleshooting methodologies
- Basic Cisco device configuration
- Understanding of physical, data link, and network layer operations
- Hands-on experience with Packet Tracer activities

## Reflection
I chose this Cisco certificate so it could help me build a strong foundation in physical networking concepts, including IP addressing, network layers, and hands-on troubleshooting. Through 14 hours of practical training with Cisco Packet Tracer, I gained experience diagnosing connectivity issues and understanding how data physically moves through cables, switches, and routers. This certificate directly supports my goal of becoming a Network Technician by giving me the practical, hardware focused skills needed to work with physical network infrastructure. Combined with my Security+, I now have a better understanding of how networks are built and how to keep them secure.
